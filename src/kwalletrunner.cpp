...
#endif
{
...
#else
    auto *overview = new QAction(QIcon::fromTheme(QStringLiteral("documentinfo")), QStringLiteral("Show Overview"));
    overview->setData(QStringLiteral("overview"));
    auto *edit = new QAction(QIcon::fromTheme(QStringLiteral("document-edit")), QStringLiteral("Edit"));
    ...
    if (!validWallet || !wallet->isOpen()) {
        KNotification::event(KNotification::Error, QStringLiteral("KWallet"), QStringLiteral("Could not open KWallet!"), QStringLiteral("kwallet"));
